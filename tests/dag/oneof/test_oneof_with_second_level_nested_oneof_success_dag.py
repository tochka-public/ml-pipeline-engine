...
async def test_oneof_with_second_level_nested_oneof_success_dag(
    ...
    assert case_oneof_node_mocker.mock.process.mock_calls == [
        ...
        call_object(_=1),
        call_object(inp=1, ds_value=1),
        call_object(feature_value=2),
        call_object(summary_value=3),
    ]
    ...
    assert case_oneof_node_mocker.mock.process.mock_calls[2].args[0].name == FeatureWithSecondLevelInputOneOf.name
    assert case_oneof_node_mocker.mock.process.mock_calls[3].args[0].name == Summary.name
    assert case_oneof_node_mocker.mock.process.mock_calls[4].args[0].name == FinishNode.name
